...
import { replayTimeline, replayCells } from '@/lib/replay';
import type { HarnessState, Run } from '@/lib/harness-types';

export default function EarthApp({ replayOnly = false, recording }: { replayOnly?: boolean; recording?: Run }) {
  ...
  }, [playing, timeline?.duration, speed]);
  useEffect(() => { if (timeline && replayMs !== null && replayMs >= timeline.duration) setPlaying(false); }, [replayMs, timeline]);
  useEffect(() => {
  ...
  async function act(action: string, selectedWidth?: string, fresh = false) {
    if (replayOnly) return;
    setBusy(true); setError(''); setPlaying(false); setReplayMs(null);
  ...
  return <main className="mx-auto flex max-w-6xl flex-col gap-6 px-4 py-8 sm:px-8">
    ...
    <div className="flex flex-wrap items-end gap-4">
      {replayOnly ? <div className="grid gap-2"><Label>Resolution</Label><span className="py-1.5 font-mono text-sm">{columns} × {rows}</span></div> : <Setting label="Resolution" value={width} onChange={value => void act('select', value)} disabled={active || busy || !connected || !!run?.inFlight} choices={[32,64,128,256,512].map(n => [String(n), `${n} × ${n / 2}`])} />}
      ...
      <div className="flex flex-wrap gap-2 sm:ml-auto">
        {active ? <Button onClick={() => void act('pause')} disabled={busy}><Pause />Pause</Button> : <>
          {timeline && !!run?.completed && <Button onClick={() => { if (playing) setPlaying(false); else { if (replayMs === null || replayMs >= (timeline?.duration ?? 0)) setReplayMs(0); setPlaying(true); } }} disabled={busy}>{playing ? <Pause /> : <Play />}{playing ? 'Pause replay' : 'Replay'}</Button>}
    ...
    <Card className="gap-0 py-0"><div className="flex flex-wrap items-center justify-between gap-3 border-b px-4 py-3"><Tabs value={mode} onValueChange={v => setMode(String(v))}><TabsList><TabsTrigger value="map">Land / water</TabsTrigger><TabsTrigger value="probability">Probability</TabsTrigger></TabsList></Tabs><div className="flex items-center gap-4 text-xs text-muted-foreground"><span><i className="mr-1.5 inline-block size-2 rounded-sm bg-[#a3d283]" />Land</span><span><i className="mr-1.5 inline-block size-2 rounded-sm bg-[#2868a0]" />Water</span><span><i className="mr-1.5 inline-block size-2 rounded-sm bg-[#55555c]" />Unanswered</span></div></div>
      <CardContent className="px-4 py-4 sm:px-6"><div className="mb-2 flex justify-between font-mono text-xs text-muted-foreground"><span>180° W</span><span>90° N</span><span>180° E</span></div><canvas ref={canvas} className="aspect-2/1 w-full cursor-crosshair rounded-sm [image-rendering:pixelated]" aria-label={`Map of ${completed} answered geographic coordinates out of ${total}. Land is green, water is blue, unanswered blocks are gray.`} onPointerMove={e => { const r = e.currentTarget.getBoundingClientRect(); const x = Math.min(columns - 1, Math.max(0, Math.floor((e.clientX - r.left) / r.width * columns))), y = Math.min(rows - 1, Math.max(0, Math.floor((e.clientY - r.top) / r.height * rows))); setHover(y * columns + x); }} onPointerLeave={() => setHover(null)} />
